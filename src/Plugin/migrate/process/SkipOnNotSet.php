...
/**
 * Skips processing the current row when the input value is not set.
 *
 * The skip_on_not_set process plugin checks to see if the current input value
 * is set. If it is not set, the further processing of the property or the
 * entire row (depending on the chosen method) is skipped and will not be
 * migrated.
 *
 * Available configuration keys:
 * - method: (optional) What action to take. Possible values:
 ...
 * process:
 *   field_type_exists:
 *     plugin: skip_on_not_set
 *     method: row
 *     source: field_name
 *     message: 'Field field_name is missed'
 * @endcode
 *
 * If source is empty, skips the entire row and the message 'Field
 ...
 * )
 */
class SkipOnNotSet extends ProcessPluginBase {
   ...
   *
   * @return mixed
   *   The input value, $value, if it is set.
   *
