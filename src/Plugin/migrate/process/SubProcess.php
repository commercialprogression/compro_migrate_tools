 ...
 *
 * The compro_compro_sub_process plugin accepts an array of associative arrays and runs 
 * each one through its own process pipeline, producing a newly keyed 
 * associative array of transformed values.
 *
 ...
 * @endcode
 *
 * The compro_sub_process process plugin will take these arrays one at a time
 * and run its own process over each one:
 *
 * @code
 * process:
 *   upload:
 *     plugin: compro_sub_process
 *     source: upload
 *     process:
 *       target_id:
 ...
 * compro_sub_process process plugin. The target_id will be found by looking up
 * the destination value from a previous migration. The display and description
 * fields will simply be mapped.
 *
 * In the next example, normally the array returned from compro_sub_process will
 * have its original keys. If you need to change the key, it is possible for the
  ...
  public function transform($value, MigrateExecutableInterface $migrate_executable, Row $row, $destination_property) {
    ...
    if (is_array($value) || $value instanceof \Traversable) {
      foreach ($value as $key => $new_value) {
        ...
        $migrate_executable->processRow($new_row, $this->configuration['process'], $new_value);
